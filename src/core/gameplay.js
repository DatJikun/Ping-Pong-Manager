...
function isSeniorPlayer(player){
  return!!player&&!player.retired&&!player.isYouth&&player.role!=='youth';
}
function activeLoanForPlayer(playerId){
  return(store.G?.loans||[]).find(loan=>loan.playerId===playerId&&!loan.returned)||null;
}
function getClubSeniorPlayers(teamId,includeLoanedOut=false){
  if(!store.G)return[];
  const result=store.G.players.filter(player=>player.teamId===teamId&&!player.loanedOut&&isSeniorPlayer(player));
  if(!includeLoanedOut)return result;
  const seen=new Set(result.map(player=>player.id));
  (store.G.loans||[]).filter(loan=>loan.fromTeamId===teamId&&!loan.returned).forEach(loan=>{
    const player=store.G.players.find(candidate=>candidate.id===loan.playerId);
    if(player&&isSeniorPlayer(player)&&!seen.has(player.id)){
      seen.add(player.id);
      result.push(player);
    }
  });
...
function matchAvailability(player,teamId){
  const unavailable=(code,reasonKey,reasonParams={})=>({available:false,code,reasonKey,reasonParams});
  if(!player)return unavailable('missing','match.nom.unavailableMissing');
  if(player.retired)return unavailable('retired','match.nom.unavailableRetired');
  if(!isSeniorPlayer(player))return unavailable('academy','match.nom.unavailableAcademy');
  const loan=activeLoanForPlayer(player.id);
  if(loan&&loan.fromTeamId===teamId){
    return unavailable('loanedOut','match.nom.unavailableLoanedOut',{club:teamName(loan.toTeamId)});
  }
  if(player.teamId!==teamId)return unavailable('notRegistered','match.nom.unavailableNotRegistered');
  if((player.injuredFor||0)>0){
    return unavailable('injured','match.nom.unavailableInjured',{rounds:player.injuredFor});
  }
  return{available:true,code:'available',reasonKey:null,reasonParams:{}};
}
...
function returnLoans(){
  ...
  store.G.loans.forEach(l=>{
    ...
    if(p){
      ...
      p.role=(l.academyLoan||l.originalRole==='youth'||p.isYouth)?'youth':'senior';
      p.loanedOut=false;
...
function getBoardObjective(){
  if(!store.G?.boardObjective)return null;
  store.G.boardObjective=normalizeBoardObjectiveChoice(store.G.boardObjective);
  return store.G.boardObjective;
}
const BOARD_CHOICE_IDS=['safe','expected','ambitious'];
function boardObjectiveChoiceId(choice,index=-1){
  if(BOARD_CHOICE_IDS.includes(choice?.id))return choice.id;
  if(BOARD_CHOICE_IDS.includes(choice?.risk))return choice.risk;
  if(choice?.failure==='soft'||Number(choice?.multiplier)<1)return'safe';
  if(choice?.failure==='fired'||Number(choice?.multiplier)>1)return'ambitious';
  return BOARD_CHOICE_IDS[index]||null;
}
function normalizeBoardObjectiveChoice(choice,index=-1){
  if(!choice)return choice;
  const id=boardObjectiveChoiceId(choice,index);
  return id&&choice.id!==id?{...choice,id}:choice;
}
...
function generateBoardObjective(teamId){
  ...
  const goal='top'+pos;
  return{id:'expected',goal,reward:Math.round((league===1?90000:45000)*goalDiff(goal)),baseOvr:teamOvr(tid),predictedPos:pos,risk:'expected',multiplier:1};
}
...
function selectBoardObjective(choiceId){
  const options=(store.G.boardObjectiveOptions||generateBoardObjectiveChoices(store.G.myTeamId)).map(normalizeBoardObjectiveChoice);
...
function generateMatchdayNews(results,myId){
  ...
  const l1sorted=leagueStandings(1);
...
function getTechContract(){
  const contract=store.G?.techContract;
  return contract&&Number(contract.yearsLeft)>0?contract:null;
}
function getTechPartnership(){
  const partnerId=getTechContract()?.partnerId;
  return partnerId?TECH_PARTNERSHIPS.find(t=>t.id===partnerId)||null:null;
}
...
function formatTechContractEffects(partnerOrContract){
  const partnerId=partnerOrContract?.partnerId||partnerOrContract?.id;
  const partner=TECH_PARTNERSHIPS.find(entry=>entry.id===partnerId)||partnerOrContract;
  const rubberId=partnerOrContract?.rubberId||partner?.rubberId;
  const rubber=EQUIPMENT.rubberProfiles[rubberId]||{};
  const modifiers=SK.filter(stat=>rubber.mods?.[stat]).map(stat=>`${SL[stat]} +${rubber.mods[stat]}`).join(' / ');
  const effects=[];
  if(modifiers)effects.push(t('equipment.effectRubber',{mods:modifiers}));
  const development=Math.round((Number(partner?.developmentBonus)||0)*100);
  if(development>0)effects.push(t('equipment.effectDevelopment',{percent:development}));
  const marketability=Math.round((Number(partner?.mktBonus)||0)*100);
  if(marketability>0)effects.push(t('equipment.effectMarketability',{percent:marketability}));
  return effects.join(' · ')||t('club.noModifiers');
}
// ── EQUIPMENT: blade + sponge are personal setups; the player club's rubber
// profile comes from its active technical contract. Mods flow through adjusted
// stats, so OVR, the match engine and every display see the same numbers.
...
function equipmentMods(p){
  ...
  if(p.teamId===store.G?.myTeamId)add(EQUIPMENT.rubberProfiles[getTechContract()?.rubberId]?.mods);
  return out;
}
function getPlayerAdjustedStats(p,teamId){
  const eq=equipmentMods(p);
  const out={};SK.forEach(s=>{out[s]=(p?.[s]||0)+(eq[s]||0);});
  return out;
}
...
function getActiveBrand(){return getTechPartnership();}
...
// Compatibility views for older callers. They are computed from the current
// five-player selection and never persisted as separate roster roles.
function myStarters(){
  const seniors=getClubSeniorPlayers(store.G.myTeamId);
  const selected=(getLastMatchSelection(store.G.myTeamId)||bestMatchSelection(store.G.myTeamId)).base
    .map(id=>seniors.find(p=>p.id===id)).filter(Boolean);
  const rest=seniors.filter(p=>!selected.some(x=>x.id===p.id)).sort((a,b)=>ovr(b)-ovr(a));
  return selected.concat(rest).slice(0,4);
}
function myReserves(){
  const firstIds=new Set(myStarters().map(p=>p.id));
  return getClubSeniorPlayers(store.G.myTeamId).filter(p=>!firstIds.has(p.id));
}
...
function teamOvr(tid){
  const best=getClubSeniorPlayers(tid).filter(p=>matchAvailability(p,tid).available)
    .sort((a,b)=>ovr(b)-ovr(a)).slice(0,4);
  return best.length?Math.round(best.reduce((s,p)=>s+ovr(p),0)/best.length):40;
}
...
function checkGoal(s){if(s.goal==='none')return true;const mt=myTeam();const myL=myLeague();const srt=leagueStandings(myL);const pos=srt.findIndex(t=>t.isPlayer)+1;if(s.goal.startsWith('top'))return pos<=(+s.goal.replace('top',''));if(s.goal.startsWith('win'))return mt.w>=(+s.goal.replace('win',''));return false;}
function sponsorProg(s){if(s.goal==='none')return{label:t('goal.progressNone'),pct:100};const mt=myTeam();const myL=myLeague();const srt=leagueStandings(myL);const pos=srt.findIndex(t=>t.isPlayer)+1;if(s.goal.startsWith('top')){const target=+s.goal.replace('top','');return{label:t('goal.positionProgress',{position:pos,target}),pct:Math.max(0,Math.min(100,100-(pos-1)*(100/target)))};}const target=+s.goal.replace('win','');return{label:t('goal.winsProgress',{wins:mt.w,target}),pct:Math.min(100,Math.round(mt.w/target*100))};}
function randName(){
...
function tuneGeneratedLeagueRoster(teamId){
  ...
  // Every club carries ten seniors (match selection plus sparring depth). Top up AI
  ...
  if(team&&!team.isPlayer&&!(team.traits||[]).includes('youthOnly')){
    while(roster.length<ROSTER_TARGET){
      ...
      np.teamId=teamId;np.role='senior';np.contractYears=1+rnd(0,2);np.nationality=store.G.countryId||'PL';
...
function ensureStaffMeta(s){
  ...
  if(s.type==='coach'&&!s.styleId)s.styleId=semanticCoachStyleId(null,s.styleName);
...
function semanticCoachStyleId(styleId,legacyStyle){
  return styleId||Object.values(COACH_STYLES).find(style=>style.label===legacyStyle)?.id||null;
}
function localizedCoachStyle(styleId,legacyStyle){
  const id=semanticCoachStyleId(styleId,legacyStyle);
  return id?t(`coachStyle.${id}`):t('staff.generalCoach');
}
function staffSnapshotDescription(snapshot){
  const hasTeam=snapshot.teamId!==null&&snapshot.teamId!==undefined&&store.G.teams.some(team=>team.id===snapshot.teamId);
  const team=hasTeam?teamName(snapshot.teamId):(snapshot.teamName||t('staff.freeMarket'));
...
function staffSnap(s){return{season:store.G.season,age:s.age||0,ovr:staffOvr(s),contractYears:s.contractYears||0,type:s.type,styleId:semanticCoachStyleId(s.styleId,s.styleName),style:s.styleName||null,teamId:s.teamId??null,teamName:s.teamId!==null&&s.teamId!==undefined?teamName(s.teamId):null};}
...
// A match needs three available seniors, drawn from the whole registered squad.
function getEligibleMatchPlayers(teamId){
  return getClubSeniorPlayers(teamId).filter(p=>matchAvailability(p,teamId).available);
}
function getSquadAvailabilityIssues(teamId){
  const healthy=getEligibleMatchPlayers(teamId);
  const injured=getClubSeniorPlayers(teamId).filter(p=>(p.injuredFor||0)>0);
...
function blockForUnavailableSquad(teamId,contextLabel){
  const issues=getSquadAvailabilityIssues(teamId);
...
function recordCoachSeason(){
  ...
  store.G.coachHistory.push({
    ...
    age:coach.age||0,
    styleId:semanticCoachStyleId(coach.styleId,coach.styleName),
    style:coach.styleName||null,
  });
}
...
function physioEffectProfile(teamId){
  const phy=getTeamPhysio(teamId);
  if(!phy)return {fatigueGainMultiplier:1,restBonus:0,injuryChanceReduction:0,injuryDurationReduction:0};
  const attribute=(value,fallback=50)=>clamp(Number.isFinite(Number(value))?Number(value):fallback,0,100)/100;
  const recovery=attribute(phy.recovery);
  const prevention=attribute(phy.prevention);
  const injuryReduction=attribute(phy.injReduction);
  const fatigueReduction=0.02+0.15*Math.pow(recovery,1.4);
  return {
    fatigueGainMultiplier:1-fatigueReduction,
    restBonus:Math.round(0.5+7*recovery),
    injuryChanceReduction:0.03+0.24*Math.pow(prevention,1.3),
    injuryDurationReduction:0.05+0.28*Math.pow(injuryReduction,1.2),
  };
}
function physioFatigueMult(teamId){
  return physioEffectProfile(teamId).fatigueGainMultiplier;
}
function physioRestBonus(teamId){
  return physioEffectProfile(teamId).restBonus;
}
function combinedRehabReduction(medicalReduction,physioReduction){
  const med=clamp(Number(medicalReduction)||0,0,1);
  const physio=clamp(Number(physioReduction)||0,0,1);
  return Math.min(0.80,1-(1-med)*(1-physio));
}
...
function roleGuaranteeLabel(promisedRole){
  return t(promisedRole==='starter'?'role.starter':promisedRole==='rotation'?'role.rotation':'role.prospect');
}
...
function genPlayer(teamId,forceAge,countryId=null){
  ...
  return ensurePlayerMeta({id,teamId,name:randNameForCountry(playerCountryId,age),age,peakAge,traits,playStyle,...stats,
    contractYears:1+rnd(0,2),salary:sal,loyalty:rnd(0,3),role:'senior',retired:false,
...
function buildMarket(){
  ...
  const loanCandidates=store.G.players.filter(p=>{
    if(p.retired||p.loanedOut||p.teamId===null||p.teamId===store.G.myTeamId||!isSeniorPlayer(p)||p.contractYears<2)return false;
    const depth=getClubSeniorPlayers(p.teamId).slice().sort((a,b)=>ovr(b)-ovr(a));
    return depth.findIndex(x=>x.id===p.id)>=5&&Math.random()<.18;
  });
...
function newGame(clubIdx, countryId){
  ...
  if(useDb&&dbPlayers.length){
    dbPlayers.forEach(raw=>{
      ...
      p.teamId=teamIndex;
      p.role=raw.role==='youth'?'youth':'senior';
  ...
  teams.forEach(t=>{
    ...
    // Ten-player senior squad: five match slots plus training depth.
...
function getMatchStarters(tid){
  const nomination=getMatchNomination(tid);
  const chosen=[...nomination.base,...nomination.reserves].filter(Boolean);
  const fallback=getEligibleMatchPlayers(tid).slice().sort((a,b)=>ovr(b)-ovr(a));
  while(chosen.length<4&&fallback.length){
    const player=fallback.shift();
    if(!chosen.some(x=>x.id===player.id))chosen.push(player);
  }
  return chosen.slice(0,4);
}
// Backwards-compatible board-order action: reorder the persistent match slots.
function moveLineup(pid,dir){
  const current=normalizeMatchSelection(getLastMatchSelection(store.G.myTeamId)||bestMatchSelection(store.G.myTeamId));
  const ids=[...current.base,...current.reserves];
...
function processSeniorUsageForTeam(teamId,playedIds){
  getClubSeniorPlayers(teamId).forEach(p=>{
    ensurePlayerMeta(p);
...
function matchNominationRules(countryId=store.G?.countryId){
  ...
  return{
    ...
    maxReserves:2,
    recommendedTotal:5,
    reservesUsedInMatch:format.protocol==='superliga',
  };
}
function normalizeMatchSelection(raw){
  const seen=new Set();
...
function bestMatchSelection(teamId){
  ...
  return{base:players.slice(0,3).map(p=>p.id),reserves:players.slice(3,5).map(p=>p.id)};
}
function getLastMatchSelection(teamId=store.G?.myTeamId){
  if(!store.G||teamId!==store.G.myTeamId||!store.G.lastMatchSelection)return null;
  return normalizeMatchSelection(store.G.lastMatchSelection);
}
function matchSelectionView(teamId,raw){
...
function validateMatchSelection(teamId,raw){
  const availableCount=getEligibleMatchPlayers(teamId).length;
  const requiredTotal=Math.min(5,availableCount);
  const unplayable=availableCount<3;
  const view=matchSelectionView(teamId,raw);
  const selectedCount=view.selectedIds.filter(id=>id!==null).length;
  const requiredSlotsFilled=!unplayable&&view.selectedIds.slice(0,requiredTotal).every(id=>id!==null);
  const ok=requiredSlotsFilled&&selectedCount===requiredTotal;
  return{
    ok,unplayable,availableCount,requiredBase:3,requiredTotal,selectedCount,view,
    reasonCode:unplayable?'tooFew':ok?null:'incomplete',
  };
}
...
function defaultAutoNomination(teamId){
  const best=bestMatchSelection(teamId);
  const byId=id=>store.G.players.find(p=>p.id===id);
  return{base:best.base.map(byId).filter(Boolean),reserves:best.reserves.map(byId).filter(Boolean)};
}
function selectAutoSeasonNomination(teamId,rawConfig=getAutoSeasonConfig()){
  ...
  const eligible=getClubSeniorPlayers(teamId).filter(p=>matchAvailability(p,teamId).available);
  ...
  const reserveFallback=defaultAutoNomination(teamId).reserves.concat(eligible);
  while(reserves.length<rules.maxReserves&&reserveFallback.length){
    const p=reserveFallback.shift();
    if(!used.has(p.id)){used.add(p.id);reserves.push(p);}
  }
...
function getMatchNomination(teamId){
  ...
  if(teamId===store.G.myTeamId&&nom&&nom.season===store.G.season&&nom.matchday===store.G.matchday){
    const check=validateMatchSelection(teamId,nom);
    if(check.ok)return{
      base:check.view.slots.slice(0,3).map(slot=>slot.player),
      reserves:check.view.slots.slice(3,5).map(slot=>slot.player).filter(Boolean),
    };
  }
  if(teamId===store.G.myTeamId&&store.G.lastMatchSelection){
    const check=validateMatchSelection(teamId,store.G.lastMatchSelection);
    if(check.ok)return{
      base:check.view.slots.slice(0,3).map(slot=>slot.player),
      reserves:check.view.slots.slice(3,5).map(slot=>slot.player).filter(Boolean),
    };
    // Interactive matchdays stop at validation and ask the manager to repair the
    // exact saved slots. Low-level simulations and diagnostics remain robust.
    return autoNomination(teamId);
  }
  return autoNomination(teamId);
}

function getSparringPartners(teamId){
  const base=(getLastMatchSelection(teamId)||bestMatchSelection(teamId)).base;
  const baseIds=new Set(base.filter(Number.isFinite));
  return store.G.players.filter(p=>{
    if(p.teamId!==teamId||p.retired||(p.injuredFor||0)>0)return false;
    if(p.role==='youth'||p.isYouth)return true;
    return isSeniorPlayer(p)&&matchAvailability(p,teamId).available&&!baseIds.has(p.id);
  });
}
// Academy players and every healthy senior outside A/B/C form the day-to-day
// sparring group. R1/R2 are part of that group when they are not on the table.
function getSparringProfile(teamId){
  const squad=store.G.players.filter(p=>p.teamId===teamId&&!p.retired);
  const partners=getSparringPartners(teamId);
...
function simTeamMatch(homeId,awayId,isCup){
  ...
  processSeniorUsageForTeam(homeId,actualForTeam(homeId));
  processSeniorUsageForTeam(awayId,actualForTeam(awayId));
...
function reserveRequestPolicy(p){
  ...
  const reasons=[];
  if(!p||!isSeniorPlayer(p)||!matchAvailability(p,p.teamId).available)reasons.push('notAvailableSenior');
...
function generateInboxForMatchday(){
  ...
  const tired=getClubSeniorPlayers(myId).filter(p=>matchAvailability(p,myId).available&&(p.fatigue||0)>75);
...
function openMatchNomination(onConfirm){
  const myId=store.G.myTeamId;
  if(getEligibleMatchPlayers(myId).length<3){toast(t('match.nom.tooFew'));return;}
  const source=getLastMatchSelection(myId)||bestMatchSelection(myId);
  const view=matchSelectionView(myId,source);
  const previousIds=[...normalizeMatchSelection(source).base,...normalizeMatchSelection(source).reserves];
  _nomState={sel:[...view.selectedIds],previousIds,onConfirm};
...
function nomToggle(pid){
  ...
  if(i>=0){_nomState.sel[i]=null;_nomState.previousIds[i]=null;}
  else{
    const vacancy=_nomState.sel.indexOf(null);
    if(vacancy<0){toast(t('match.nom.maxFive'));return;}
    _nomState.sel[vacancy]=pid;
    _nomState.previousIds[vacancy]=pid;
  }
  renderNominationModal();
}
function nomClear(){
  if(!_nomState)return;
  _nomState.sel=Array(5).fill(null);
  _nomState.previousIds=Array(5).fill(null);
  renderNominationModal();
}
function nomBest(){
  if(!_nomState)return;
  const best=normalizeMatchSelection(bestMatchSelection(store.G.myTeamId));
  _nomState.sel=[...best.base,...best.reserves];
  _nomState.previousIds=[..._nomState.sel];
  renderNominationModal();
}
function nomConfirm(){
  if(!_nomState)return;
  const selection={base:_nomState.sel.slice(0,3),reserves:_nomState.sel.slice(3,5)};
  const check=validateMatchSelection(store.G.myTeamId,selection);
  if(!check.ok){toast(t(check.unplayable?'match.nom.tooFew':'match.nom.chooseThree'));return;}
...
function renderNominationModal(){
  const myId=store.G.myTeamId;
  const pool=getClubSeniorPlayers(myId,true).slice().sort((a,b)=>{
    const aSelected=_nomState.sel.indexOf(a.id),bSelected=_nomState.sel.indexOf(b.id);
    if(aSelected>=0||bSelected>=0)return(aSelected<0?99:aSelected)-(bSelected<0?99:bSelected);
    const availableDiff=Number(matchAvailability(b,myId).available)-Number(matchAvailability(a,myId).available);
    return availableDiff||ovr(b)-ovr(a)||a.name.localeCompare(b.name);
  });
  const raw={base:_nomState.previousIds.slice(0,3),reserves:_nomState.previousIds.slice(3,5)};
  const view=matchSelectionView(myId,raw);
  const check=validateMatchSelection(myId,{base:_nomState.sel.slice(0,3),reserves:_nomState.sel.slice(3,5)});
  const slotLabels=['match.nom.slotA','match.nom.slotB','match.nom.slotC','match.nom.slotR1','match.nom.slotR2'].map(key=>t(key));
  const reason=status=>status?.reasonKey?t(status.reasonKey,status.reasonParams):'';
  const modal=document.getElementById('modal');modal.className='modal modal-lg';
  ...
  ${pool.map(p=>{const idx=_nomState.sel.indexOf(p.id);const tag=idx>=0?slotLabels[idx]:null;const status=matchAvailability(p,myId);
    return `<div ${status.available?`onclick="nomToggle(${p.id})"`:''} style="display:grid;grid-template-columns:auto 1fr auto auto;gap:10px;align-items:center;padding:8px 10px;border:1px solid ${tag?(idx<3?'var(--g)':'var(--blue)'):'var(--b1)'};background:${tag?'var(--s2)':'var(--s1)'};border-radius:6px;cursor:${status.available?'pointer':'not-allowed'};opacity:${status.available?1:.58}">
...
function applyResult(r){
  ...
  if(!r.forfeit)tryInjuriesAfterMatch(r);
...
function tryInjuriesForTeam(teamId,participantIds=null){
  const injured=[];
  ...
  const physioEffects=physioEffectProfile(teamId);
  const fallbackIds=new Set([...bestMatchSelection(teamId).base,...bestMatchSelection(teamId).reserves].filter(Number.isFinite));
  const eligibleIds=participantIds instanceof Set?participantIds:fallbackIds;
  let fieldable=getClubSeniorPlayers(teamId).filter(p=>matchAvailability(p,teamId).available).length;
  getClubSeniorPlayers(teamId).filter(p=>p.injuredFor===0&&eligibleIds.has(p.id)).forEach(p=>{
    ...
    baseChance*=(1-physioEffects.injuryChanceReduction);
    if(Math.random()<baseChance){
      const baseDur=rnd(1,5);
      const rehabReduction=combinedRehabReduction(medBonus,physioEffects.injuryDurationReduction);
      const dur=Math.max(1,Math.round(baseDur*(1-rehabReduction)));
...
function tryInjuriesAfterMatch(resultOrHomeId,awayId){
  const out=[];
  const result=resultOrHomeId&&typeof resultOrHomeId==='object'?resultOrHomeId:null;
  const homeId=result?result.homeId:resultOrHomeId;
  const actualAwayId=result?result.awayId:awayId;
  const homeParticipants=new Set(),awayParticipants=new Set();
  (result?.matchups||[]).forEach(matchup=>{
    (matchup.homePair||[matchup.homePlayer]).filter(Number.isFinite).forEach(id=>homeParticipants.add(id));
    (matchup.awayPair||[matchup.awayPlayer]).filter(Number.isFinite).forEach(id=>awayParticipants.add(id));
  });
  if(homeId!=null)out.push(...tryInjuriesForTeam(homeId,result?homeParticipants:null));
  if(actualAwayId!=null&&actualAwayId!==homeId)out.push(...tryInjuriesForTeam(actualAwayId,result?awayParticipants:null));
  return out;
}
...
function applyGrowth(){
  ...
  store.G.players.forEach(p=>{
    ...
    // club the usual way: the graduate joins the senior squad and his contract then
    ...
    if(p.role==='youth'&&p.age>=21&&p.teamId!==null){
      ...
      if(keep){
        ...
        p.teamId=null;p.role='senior';p.isYouth=false;p.contractYears=0;
    ...
    if(p.teamId===myId)devMult*=1+(getTechPartnership()?.developmentBonus||0);
    const roleMult=isYouthAcademy?0.8:1.0;
...
function updateRecords(){
  ...
  const recordTeam=mt||leagueStandings(1)[0];
...
function giveSeasonAwards(){
  const myId=store.G.myTeamId;
  const allSeniors=store.G.players.filter(p=>!p.retired&&p.teamId!==null&&isSeniorPlayer(p));
  const awards=[];
  if(allSeniors.length){
    ...
    const eligible=allSeniors.filter(p=>appearances(p)>=Math.ceil((teamMatches.get(p.teamId)||0)*0.75)&&appearances(p)>0);
    const awardPool=eligible.length?eligible:allSeniors.filter(p=>appearances(p)>0);
  ...
  [1,2].forEach(league=>{
    const srt=leagueStandings(league);
    const champ=srt[0];
    if(league===1){
      getClubSeniorPlayers(champ.id).forEach(p=>{p.awards=p.awards||[];p.awards.push({season:store.G.season,type:'league_champion',clubName:champ.name,displayLabel:'Mistrz I Ligi',label:`Mistrz I Ligi S${store.G.season}`});if(p.teamId===myId)awards.push({player:p.name,club:champ.name,type:'league_champion',label:'Mistrz I Ligi'});});
    }
  });
  return awards;
}
...
function doPromotionRelegation(){
  const l1=leagueStandings(1);
  const l2=leagueStandings(2);
...
// CUP ROUND PLAY
// \u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550
const CUP_PRIZES={winner:35000,finalist:18000,semifinalist:9000,quarterfinalist:4500};
function getCupClubStatus(teamId){
  const cup=store.G?.cup;if(!cup)return{state:'waiting',nextTrigger:4,roundIndex:0,path:[],opponent:null,result:null};
...
async function playCupRound(){
  ...
  if(blockForUnavailableSquad(store.G.myTeamId,'match.cupBlocked'))return;
  if(!ui.autoPlay){
    const current=store.G.matchNomination&&store.G.matchNomination.season===store.G.season&&store.G.matchNomination.matchday===store.G.matchday
      ?store.G.matchNomination:store.G.lastMatchSelection;
    if(!current||!validateMatchSelection(store.G.myTeamId,current).ok){
      openMatchNomination(()=>playCupRound());
      return;
    }
  }
...
async function playCupRoundBody(){
  ...
  addLog(plural('cupLog.header',round.length,{round:roundName}),'hl');
  ...
  if(nextRound.length<=1){
    ...
    const cupPrizeTable={winner:CUP_PRIZES.winner,finalist:CUP_PRIZES.finalist,semifinal:CUP_PRIZES.semifinalist,quarterfinal:CUP_PRIZES.quarterfinalist};
    ...
    if(cup.winner.isReal&&cup.winner.id===myId){
      ...
      getClubSeniorPlayers(myId).forEach(p=>{
        p.awards=p.awards||[];
    ...
    addLog(plural('cupLog.nextRound',nextRound.length),'hl');
  }
  ...
  const html=`<div class="vme">
    ...
    <div class="vme-cards">
      ...
      <div class="vme-center">
        <div class="vme-match-index">${t('vme.matchIndex',{current:currentIdx+1,total:matchups.length})}</div>
...
async function runMatchday(){
  ...
  if(blockForUnavailableSquad(store.G.myTeamId,'match.matchdayBlocked'))return;
  if(getEligibleMatchPlayers(store.G.myTeamId).length<3){
    blockForUnavailableSquad(store.G.myTeamId,'match.matchdayBlocked');
    return;
  }
  const myL=myLeague();
  ...
  if(!ui.autoPlay){
    const current=store.G.matchNomination&&store.G.matchNomination.season===store.G.season&&store.G.matchNomination.matchday===store.G.matchday
      ?store.G.matchNomination:store.G.lastMatchSelection;
    const check=current?validateMatchSelection(store.G.myTeamId,current):null;
    if(!check?.ok){
      openMatchNomination(()=>runMatchday());
      return;
    }
    if(!store.G.matchNomination){
      const saved=normalizeMatchSelection(current);
      store.G.matchNomination={season:store.G.season,matchday:store.G.matchday,base:saved.base,reserves:saved.reserves};
    }
  }
  ...
  if(getEligibleMatchPlayers(myId).length<3){
    ui.running=false;
    blockForUnavailableSquad(myId,'match.matchdayBlocked');
    return;
  }
  ...
  if(otherMatches){
    addLog(plural('matchLog.otherDivision',otherMatches.length,{division:otherL===1?'I':'II'}),'dm');
  }
...
function aiPoachOutgrownStars(){
  ...
  store.G.teams.filter(t=>!t.isPlayer).forEach(seller=>{
    ...
    const topFiveIds=new Set(getClubSeniorPlayers(seller.id).slice().sort((a,b)=>ovrBase(b)-ovrBase(a)).slice(0,5).map(p=>p.id));
    store.G.players.filter(p=>p.teamId===seller.id&&!p.retired&&!p.isYouth&&p.role!=='youth'&&!p.loanedOut).forEach(p=>{
      ...
      const leagueStepUp=seller.league===2&&topFiveIds.has(p.id)&&ovrBase(p)>=Math.max(78,l2Avg);
      ...
      const buyer=store.G.teams
        ...
        .filter(t=>{
          ...
          const weakestSenior=senior.sort((a,b)=>ovrBase(a)-ovrBase(b))[0];
          return senior.length<10||!weakestSenior||ovrBase(p)>ovrBase(weakestSenior);
        })
        .sort((a,b)=>(b.budget||0)-(a.budget||0))[0];
      if(!buyer)return;
      buyer.budget-=fee;seller.budget=(seller.budget||0)+fee;
      p.teamId=buyer.id;p.salary=contractExpect(p,buyer.id).salary;p.contractYears=1+rnd(0,2);p.role='senior';
      ...
      while(store.G.players.filter(x=>x.teamId===buyer.id&&!x.retired&&x.role!=='youth').length>10&&surplus.length){
        ...
        released.teamId=null;released.contractYears=0;released.role='senior';
      }
...
function offseasonFatigue(fatigue){
  return Math.round(Math.max(0,(Number(fatigue)||0)-30)*0.20);
}
function endSeason(){
  ...
  // A contract settles its signed cashflow once per season and remains active
  // until all signed years have been consumed. Never re-read catalogue prices:
  // existing careers retain the deal they actually signed.
  const techContract=getTechContract();
  if(techContract){
    const delta=Number(techContract.annualCashflow)||0;
    const partner=TECH_PARTNERSHIPS.find(entry=>entry.id===techContract.partnerId);
    myTeam().budget+=delta;
    const finance=ensureSeasonFinance();
    ...
    if(delta>0)safeLog(t('season.techPartnershipLog',{name:partner?.name||techContract.partnerId,amount:`+${formatCurrency(delta)}`}),'gd');
    else if(delta<0)safeLog(t('season.techPartnershipLog',{name:partner?.name||techContract.partnerId,amount:formatCurrency(delta)}),'bd');
    techContract.yearsLeft=Math.max(0,Math.trunc(Number(techContract.yearsLeft)||0)-1);
    if(techContract.yearsLeft>0)store.G.techPartnership=techContract.partnerId;
    else{store.G.techContract=null;store.G.techPartnership=null;}
  }
  ...
  store.G.players.forEach(p=>{
    ...
    p.injuredFor=0;p.fatigue=offseasonFatigue(p.fatigue);
...
// v13: Require exactly 3 sponsors. A multi-year equipment contract carries into
// preseason and therefore fulfils the technical-partnership requirement itself.
function startSeason(){
  ...
  if(!getTechContract()){toast(t('season.needTechPartner'));return;}
...
function rebalanceAiLineup(teamId){
  ...
  roster.forEach(p=>{
    p.role=(p.isYouth||p.role==='youth')?'youth':'senior';
  });
}
function aiSignPlayers(){
  ...
  aiTeams.forEach(team=>{
    ...
    let roster=getClubSeniorPlayers(team.id).filter(p=>p.contractYears>0);
    ...
    roster=getClubSeniorPlayers(team.id).filter(p=>p.contractYears>0);
    const weakestSenior=roster.slice().sort((a,b)=>ovrBase(a)-ovrBase(b))[0];
    const futureTarget=youthOnly?null:contractedTargets.find(({item,player})=>{
      ...
      if(weakestSenior&&ovrBase(player)<ovrBase(weakestSenior)+2&&playerCeiling(player)<playerCeiling(weakestSenior)+3&&roster.length>=desiredRoster)return false;
      return true;
    });
    if(futureTarget){
...
function openPlayerModal(pid,pendingSource,pendingIndex){
  ...
  <div class="g2 gp12 mb14">
    <div>
      ...
      ${p.equipment?`<div class="mt-8 pd8-10 bb1 r10 bgs1 fs11">
        ...
        ${p.teamId===store.G.myTeamId?`<div class="mt-3 ink3">${t('player.partnerRubber')}: ${partnerRubber?t(`equipment.rubber.${partnerRubber}`):t('common.none')}${partnerRubber?` (${getTechPartnership()?.name||t('common.none')})`:''}</div><div class="mt-3">${t('player.partnerRubberModifiers')}: ${formatEquipmentMods(partnerRubberMods)}</div>`:''}
      </div>`:''}
  ...
  <div class="g2 gp10 mb14">
    <div class="tile tile-lg">
      ...
      <div class="pnl-row"><div>${t('player.matchEquipmentEffect')}</div><div class="${mods.equipmentOvr>0?'pnl-pos':''}">${mods.equipmentOvr>0?`+${mods.equipmentOvr}`:'0'}</div></div>
...
function sellPlayer(pid){
  ...
  p.teamId=best.team.id;p.role='senior';p.isYouth=false;p.contractYears=Math.max(2,p.contractYears||0);
...
function techContractAnnualCashflow(partner,termYears){
  const years=clamp(Math.round(Number(termYears)||1),1,3);
  const base=Number(partner?.annualCashflow??partner?.costPerSeason)||0;
  const multiplier=base<0?1-(years-1)*0.04:1+(years-1)*0.03;
  return Math.round(base*multiplier);
}
function techContractBreakFee(contract){
  if(!contract)return 0;
  const yearsLeft=Math.max(0,Math.trunc(Number(contract.yearsLeft)||0));
  const base=Math.max(2500,Math.abs(Number(contract.annualCashflow)||0)*0.75+1000*yearsLeft);
  return Math.ceil(base/500)*500;
}
function terminateTechPartnership(){
  const contract=getTechContract();
  if(!contract)return false;
  const fee=techContractBreakFee(contract);
  if(!confirm(t('sponsor.techTerminationConfirm',{fee:formatCurrency(fee)})))return false;
  const club=myTeam();
  if(!club||(club.budget||0)<fee){toast(t('sponsor.techTerminationInsufficient',{fee:formatCurrency(fee)}));return false;}
  club.budget-=fee;
  const finance=ensureSeasonFinance();if(finance)finance.brandCosts+=fee;
  store.G.techContract=null;store.G.techPartnership=null;
  render();updateHeader();toast(t('sponsor.techTerminated',{fee:formatCurrency(fee)}));persistGame();
  return true;
}
function selectTechPartnership(tpId,years=1){
  const tp=TECH_PARTNERSHIPS.find(entry=>entry.id===tpId);if(!tp)return false;
  if(store.G.phase!=='preseason'){toast(t('sponsor.techPreseasonOnly'));return false;}
  if(store.G.techContract){toast(t('sponsor.techContractLocked'));return false;}
  const pres=calcPrestige();
  if(pres<tp.prestige[0]||pres>tp.prestige[1]){toast(t('sponsor.prestigeMismatch',{prestige:pres}));return false;}
  const termYears=clamp(Math.round(Number(years)||1),1,3);
  store.G.techContract={
    partnerId:tp.id,rubberId:tp.rubberId,termYears,yearsLeft:termYears,
    signedSeason:store.G.season,annualCashflow:techContractAnnualCashflow(tp,termYears),
  };
  store.G.techPartnership=tp.id;
  render();
  const cashflow=store.G.techContract.annualCashflow;
  const costStr=cashflow>0?'+'+formatCurrency(cashflow):formatCurrency(cashflow);
  toast(t('sponsor.partnershipSelected',{name:tp.name,bonus:formatTechContractEffects(store.G.techContract),cost:costStr}));
  persistGame();
  return true;
}
...
function checkScoutReturns(){
  (store.G.scoutMissions||[]).forEach(m=>{
    if(!m.done&&store.G.matchday>=m.startMatchday+m.duration){
      ...
      toast(plural('scout.returned',numPlayers,{name:scout.name}));
    }
  });
}
...
async function runTop12MastersBody(leagueId){
  ...
  addLog(plural('top12.format',participants.length));
  await sleep(600);
...
window.PPM.gameplay = { sleep, rnd, compareLeagueTeams, leagueStandings, getLoanedOut, getLoanedIn, isSeniorPlayer, getClubSeniorPlayers, matchAvailability, canLoanOut, openLoanModal, doLoanOut, doBorrowIn, returnLoans, getMerchIncome, estimateAttendance, ticketPriceDemand, calcTVRights, getPRDirector, getPRDirectorMarket, getRivalPRDirectors, getTeamPRDirector, hirePRDirector, genNewsFeed, pushNews, generateMatchdayNews, getTechContract, getTechPartnership, formatTechContractEffects, techContractAnnualCashflow, techContractBreakFee, terminateTechPartnership, ovr, ovrBase, engineStats, equipmentMods, getPlayerAdjustedStats, getActiveBrand, myTeam, myPlayers, myStarters, myReserves, teamName, playerName, teamLeague, myLeague, teamOvr, getMax, phaseLabel, phaseColor, seasonFormLabel, staffOvr, ratingProfile, staffOvrColor, safeLog, calcPrestige, goalDiff, goalDesc, checkGoal, sponsorProg, contractExpect, negResponse, roleGuaranteeLabel, getNextSeasonCommitments, awardLabel, randName, totalWages, totalWageBreakdown, getMyScouts, getPolishClubStaffMarket, getAllExternalStaffMarket, calcTeamMorale, moraleLabel, calcLeagueMaint, facilityUpkeep, snap, calcGoat, genPlayer, genYouthPlayer, myYouth, promoteYouth, staffSalary, staffEffectiveBonus, genStaff, clubStaffQuality, genSponsorOffers, genScoutPool, staffMarketPolicy, freeAgentMarketPolicy, updateFreeAgentLifecycle, buildMarket, toggleMarketShortlist, toggleMarketCompare, makeSchedule, genCupBracket, newGame, getMatchStarters, moveLineup, getCoach, effectiveRating, simIndividual, simTeamMatch, simCupMatch, applyResult, tryInjuries, tryInjuriesForTeam, tryInjuriesAfterMatch, getTeamPsychologist, psychMatchBoost, getTeamPhysio, physioEffectProfile, physioFatigueMult, physioRestBonus, combinedRehabReduction, getStyleEdge, buildPointSimProfile, getLivePointStats, applyLongRallyFatigue, coachDevMultiplier, tickInjuries, applyGrowth, retirePlayer, updateRecords, giveSeasonAwards, doPromotionRelegation, buildMatchProgression, buildBudgetEntry, shouldPlayCup, playCupRound, initCanvasVME, stopCanvasVME, renderVME, matchdayModalTitle, runMatchday, safeCloseMatchday, autoPlaySeason, normalizeAutoSeasonConfig, getAutoSeasonConfig, setAutoSeasonConfig, selectAutoSeasonNomination, prepareAutoSeasonDecisions, endSeason, startSeason, aiSignPlayers, aiInfrastructureInvestmentChance, acceptClubOffer, getFilteredClubOffers, setClubOfferFilter, refreshClubOfferPicker, openClubOfferPicker, showPostSeasonGala, pullYouth, signAcademyProspect, genAcademyIntake, runAcademyMiniTournament, signTrialProspect, resolvePlayerProfile, openPlayerModal, negUpdate, openNegotiate, doNegotiate, releasePlayer, openStaffModal, openStaffNeg, doHireStaff, fireStaff, upgradeInfra, downgradeInfra, academyUpkeep, sellPlayer, youthSaleValue, youthSaleInterest, selectTechPartnership, signSponsor, signSponsorPreseason, genScoutPlayer, sendScout, checkScoutReturns, hireScout, scoutSign, shouldPlayTop12, getTop12Participants, openTop12Picker, simIndividualTournamentMatch, runTop12Masters, miniChart, calcTeamMarketability, calcPlayerMarketability, getBoardObjective, boardObjectiveLabel, boardObjectiveChoiceId, normalizeBoardObjectiveChoice, generateBoardObjective, generateBoardObjectiveChoices, selectBoardObjective, difficultyEffectsSummary, getClubHistory, openTeamOverview, getAvatarData, getTeamLogoData, getTeamBranding, playerCeiling, staffCeiling, styleLabel, pruneCareerData, hofRankScore, playerWageForOvr, staffWageForOvr, staffNegResponse, staffNegUpdate, findStaffById, leagueStrengthTopForBudget, coachDevPercent, genPrincipal, principalLifecycle, assignAiPrincipal, principalStrategyLabel, handleManagerFired, pushMail, unreadMailCount, pendingDecisions, markMailRead, answerMail, generateInboxForMatchday, reserveRequestPolicy, settleMatchPromises, openMatchNomination, nomToggle, nomClear, nomBest, nomConfirm, getMatchNomination, autoNomination, matchNominationRules, normalizeMatchSelection, getLastMatchSelection, bestMatchSelection, matchSelectionView, validateMatchSelection, getSparringProfile, getMatchPreparation, getEligibleMatchPlayers, replenishStaffPools, runSeededEvent, makeDoublesPair, getLeagueFormat, tablePointsFor, protocolDescription, peakAgeFor, fitEquipmentToStyle, simulateBackgroundSeasons, maintainAiRosters };
window.PPM.gameplay.offseasonFatigue=offseasonFatigue;
window.PPM.gameplay.CUP_PRIZES=CUP_PRIZES;
window.PPM.gameplay.getCupClubStatus=getCupClubStatus;
window.PPM.gameplay.getLeagueStrengthTargets=getLeagueStrengthTargets;
})();
