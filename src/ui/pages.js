...
${clubOffers.length&&store.G.phase!=='pre'?`<div class="card mb14 bt3-blue"><div class="ct">${t('dash.clubOfferTitle').toUpperCase()}</div><div class="grid gp12 aic" style="grid-template-columns:1.1fr auto"><div><div class="b7">${t('dash.clubOfferReach',{eligible:eligibleClubOffers.length,total:clubOffers.length})}</div><div class="fs11 ink3 mt-4" style="line-height:1.5">${t('dash.clubOfferHint')}</div></div><button class="btn pr" onclick="openClubOfferPicker()">${t('dash.chooseClub').toUpperCase()}</button></div></div>`:''}
  ...
  <div class="g2">
    ...
    <div>
      ...
      ${store.G.cup?(()=>{const status=window.PPM.gameplay.getCupClubStatus(mt.id);return`<div class="card"><div class="ct">${t('dash.cup').toUpperCase()}</div><div class="fs12 ink3">${t(`cup.status.${status.state}`)}</div>${status.state!=='due'&&status.nextTrigger!==null?`<div class="fs10 cpurple mt-6 b7">${t('cup.nextTrigger',{matchday:status.nextTrigger})}</div>`:''}</div>`;})():''}
...
function pageCup(){
  ...
  <div class="card mb14"><div class="fs12 ink3">${t('cup.format')}</div><div class="fs11 mt-6">${t('cup.prizes',{winner:formatCurrency(window.PPM.gameplay.CUP_PRIZES.winner),finalist:formatCurrency(window.PPM.gameplay.CUP_PRIZES.finalist),semifinalist:formatCurrency(window.PPM.gameplay.CUP_PRIZES.semifinalist),quarterfinalist:formatCurrency(window.PPM.gameplay.CUP_PRIZES.quarterfinalist)})}</div>${(()=>{const status=window.PPM.gameplay.getCupClubStatus(myId);const names=['1/16','1/8',t('cup.quarterfinal'),t('cup.semifinal'),t('cup.final')];const path=status.path.map(row=>`${names[row.roundIndex]}: ${row.opponent?.name||t('common.none')}${row.result?` ${row.result.score}`:''}`).join(' · ');return`<div class="fs11 mt-6 b7">${t(`cup.status.${status.state}`)}${status.state!=='due'&&status.nextTrigger!==null?` ${t('cup.nextTrigger',{matchday:status.nextTrigger})}`:''}</div>${path?`<div class="fs11 mt-4">${t('cup.path',{path})}</div>`:''}`;})()}</div>
...
function renderMainMenu(){
  ...
  return`<div class="flex fdc aic jcc pd20" style="min-height:calc(100vh - 40px);gap:26px">
    <div class="tac">
      ...
      <div class="fs10 ink3 mt-8">${window.PPM.appVersionLabel?.()||'0.1.1 beta'}</div>
    </div>
